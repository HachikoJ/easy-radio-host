...
目标是个人主题电台：开始收听、看清当前内容、随时点歌、找回喜欢的歌。保留 FastAPI + DeepSeek + Qwen/MiniMax/edge-tts + 现有音乐代理，首轮不新增运行依赖。
...
| 已完成 | 同步歌词与播放状态动效 | 提供真实 LRC 逐行同步、跳播、时间偏移、译文及缺失降级，详见 README。参考 Qiaomu 等通用交互独立实现，不伪造逐字精度或实时频谱 |
| 已完成 | 口味与场景进入推荐上下文 | 自动节目结合人工主题标签、可选收藏偏好、同歌手延伸与全库探索，程序选曲后再生成口播。详见 [Embeat 参考与推荐规则](EMBEAT-ADAPTATION.md)。主动点歌保持原行为；天气/日历没有接入 |
