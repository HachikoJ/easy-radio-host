...
class FrontendContract(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_scheduled_theme_is_ready_before_the_module_app_runs(self):
        ...
        self.assertIn('class="theme-booting app-booting" data-appearance="dark"', html)
        ...
        self.assertIn('<body id="top" class="quiet-ui">', html)
        self.assertNotIn('class="sidebar"', html)
        header = html[html.index('<header class="page-header">'):html.index('</header>')]
        self.assertIn('class="brand"', header)
        self.assertIn('<nav aria-label="主导航">', header)
        ...
        style = (ROOT / "backend" / "static" / "style.css").read_text()
        listening_style = (ROOT / "backend" / "static" / "listening.css").read_text()
        self.assertNotIn("classList.add('quiet-ui')", layout)
        self.assertNotIn(".sidebar", style)
        self.assertNotIn("--sidebar", style)
        self.assertNotIn(".sidebar", listening_style)
        self.assertIn("requestAnimationFrame(() => requestAnimationFrame(() => {", layout)
        self.assertIn("window.addEventListener('tingjian:record-ready', reveal", layout)
        lyrics = (ROOT / "backend" / "static" / "lyrics.js").read_text()
        self.assertIn("record-stage record-booting", lyrics)
        record_motion = (ROOT / "backend" / "static" / "record-motion.js").read_text()
        self.assertIn("tingjian:record-ready", record_motion)
        self.assertNotIn("revealFallback", record_motion)
