async (page) => {
  ...
  await page.addInitScript(() => {
    ...
    proto.play = function () {
      ...
      if (url.includes('/announcement/') && !window.holdAnnouncement) {
        setTimeout(() => { if (this.src === url && !this.paused) this.dispatchEvent(new Event('ended')); }, 250);
      }
      return Promise.resolve();
    };
  });
  let resolveStatus = 'limited', intentStatus = 'unavailable', availabilityStatus = 'ready', showFailure = null, songIds = ['bad', 'good'];
  let showDelay = 0, intentDelay = 0;
  let showCalls = 0, resolveCalls = 0, availabilityCalls = 0;
  ...
  await page.route('**/api/show', async route => {
    ...
    return route.fulfill(showFailure ? { status: 429, json: { detail: { status: showFailure, notice: '本轮音源访问额度已用完，将自动继续。', retry_after: 2 } } } : { json: { items: songIds.map(song) } }).catch(() => {});
  });
  ...
  showFailure = 'limited'; await reset(); await page.locator('#generate').click();
  await page.waitForFunction(() => document.querySelector('#show-status').textContent === '等待自动续播');
