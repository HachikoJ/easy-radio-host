...
- **Online library:** Verify actual audio before adding recommendations or requests to the queue. Search across channels using title, artist, and recording version, with simplified/traditional Chinese matching, multiple candidates, and pagination. Playback failures first refresh the URL, then search for another matching source. Exhausted searches show an explicit notice before advancing; rate limits, timeouts, and incomplete searches remain retryable and are not reported as missing songs.
...
The 10 channels listed by GD Studio are searched according to actual availability; they are not all continuously available. Search, URL, and lyric requests share a local budget of at most 50 requests per five minutes, with caching, concurrent request deduplication, and cooldown for unsupported channels. Upstream limits may still apply. Only verified tracks enter the queue; later network changes and URL expiration still require playback recovery. See [source resolution and recovery](docs/MUSIC-SOURCES.md).
