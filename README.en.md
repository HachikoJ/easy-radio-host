...
- **Playback animation:** A rotating disc displays a theme image at its center, surrounded by animated radial bars. Motion follows actual playback and stops during buffering, pauses, or when the page is hidden. The animation toggle is remembered and system reduced-motion preferences are respected. The image is not the track's album artwork, and the CSS animation is not a real-time audio spectrum.
