...
- **专注收听**：统一的收听页随视口高度分配唱片与歌词空间，主要内容保持一屏；待机和节目生成时收起空歌词区，播放后恢复双栏歌词。歌曲标题与右对齐的跟随、动效和设置同处一行。主题电台、节目/收藏/历史与点歌互动按需展开，底部播放控制始终保留播放上下文。明暗配色在没有保存偏好时跟随系统；支持空格播放、左右键快进退、上下键调音量及兼容浏览器的系统媒体控制。右上角的访问官网、致谢和作者 GitHub 在新标签页打开，致谢默认中文，点击 English 后展示英文。
- **收藏与历史**：浏览器本地保存最多 100 首收藏、50 条按标题去重的收听历史；歌曲实际开始播放后才记入历史。
- **点歌互动**：与小蓝聊天、点歌或控制播放；支持生成取消、失败重试和失败时保留输入草稿。
- **在线曲库**：推荐和点歌入队前检查真实音频；按歌名、歌手和版本跨渠道检索，支持繁简匹配、多个候选及分页。播放失败先刷新，再寻找其他匹配音源；用户指定歌曲无可用匹配时明确提示曲名并播报原因。限流、超时和检索未完成分别说明，不误报歌曲不存在。
- **同步歌词**：歌曲开始后展开大面积歌词区，以淡化的主题照片为背景，用文字高亮呈现当前行，歌词在区域内独立滚动；主持人讲话时在同一区域展示口播文稿。加载、确实无歌词和请求失败会在区域中央清楚呈现。底部播放器显示当前歌词，点按可聚焦歌词区并恢复跟随。LRC 默认跟随播放，点按可跳播（含 0 秒）；手动浏览后 3 秒恢复跟随，只有主动关闭跟随开关才持续停止。支持 -10 至 +10 秒延后调整和可选的时间戳译文。歌词请求遇到 429 时按服务返回的等待时间自动恢复；只有原渠道成功返回 200 但歌词为空时，才按歌名、歌手和版本严格匹配其他渠道。成功结果只在内存缓存，不保存歌词文件。加载、等待额度、跨渠道补找、确实无歌词、请求失败及超时分别提示，纯文本不猜测时间。
...
GD Studio 列出的 10 个渠道会按实际开放情况参与检索，并不代表全部渠道始终可用。搜索、取链和歌词共享滚动 300 秒最多 45 次的本地请求预算，SQLite 跨进程保存计数与冷却，重启不会重置；缓存命中与并发合并不重复消耗额度。上游返回 429 时遵守完整 `Retry-After`，不通过切换渠道继续请求。已缓存的地址仍须通过音频校验才能播放；无可播缓存时播放预生成陪伴话题，到期立即恢复音乐。歌词限流按 `Retry-After` 自动续取，200 空结果才会严格跨渠道补找。详见 [音源检索与恢复](docs/MUSIC-SOURCES.md)。
...
| 主应用 :8100 | GET | `/api/playback/cooldown/<version>/<id>.mp3` | 播放版本化的限流陪伴音频 |
...
无可播歌曲时，`/api/show` 的错误 `detail` 为结构对象，包含原因与恢复信息；自建客户端不应只按字符串处理。`/api/intent` 的音源失败响应保留 `items/actions`，提供 `notice/availability` 并包含 `next` 动作；先显示并播报原因，再执行下一曲或等待恢复。歌词接口保持旧路径兼容，可选的 `title/artist` 用于跨渠道身份匹配；成功返回 `{lyric, translation, source}`，其中 `source` 是实际歌词渠道。限流返回 HTTP 429、`Retry-After` 头及 `detail.status/retry_after`，客户端须保持“等待额度”状态并在到期后续取，不能显示为确实无歌词。升级与回退时的额度状态处理见[部署手册](DEPLOY-HANDOFF.md#7-更新与恢复)。
