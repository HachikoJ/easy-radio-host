...
- **同步歌词**：底部播放器显示当前歌词，顶部歌词入口或底部歌词可直接打开沉浸视图。LRC 逐行高亮、点按跳播（含 0 秒）、跟随开关和 -10 至 +10 秒延后调整；手动滚动停止跟随，有时间戳的译文可选择显示。加载、无歌词、请求失败及超时分别提示，纯文本不猜测时间。
- **播放动效**：播放区、歌词区与底部播放器呈现清晰的 CSS 律动条，随实际播放启停，等待、暂停或页面隐藏时停止；记住开关并明确显示系统减少动态效果状态，不是实时频谱。
