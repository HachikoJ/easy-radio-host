...
- **连续播放**：节目单、上一段与下一段、进度和音量控制，支持自动续播与停止。无音源、限流或临时故障时，文字提示并播报原因后自动尝试下一首推荐；暂无可播歌曲时等待恢复并自动续播。暂停或关闭页面会取消请求、等待计时及播报，释放音频连接。
...
GD Studio 列出的 10 个渠道会按实际开放情况参与检索，并不代表全部渠道始终可用。搜索、取链和歌词共享滚动 300 秒最多 45 次的本地请求预算，SQLite 跨进程保存计数与冷却，重启不会重置；缓存命中与并发合并不重复消耗额度。上游返回 429 时遵守完整 `Retry-After`，不通过切换渠道继续请求。已缓存的地址仍须通过音频校验才能播放；无可播缓存时播报等待原因，到期自动恢复。详见 [音源检索与恢复](docs/MUSIC-SOURCES.md)。
...
无可播歌曲时，`/api/show` 的错误 `detail` 为结构对象，包含原因与恢复信息；自建客户端不应只按字符串处理。`/api/intent` 的音源失败响应保留 `items/actions`，提供 `notice/availability` 并包含 `next` 动作；先显示并播报原因，再执行下一曲或等待恢复。升级与回退时的额度状态处理见[部署手册](DEPLOY-HANDOFF.md#7-更新与恢复)。
